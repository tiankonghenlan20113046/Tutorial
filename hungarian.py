...
class Hungarian:
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        ...
        # Step 1: Subtract row mins from each row.从每一行中减去这行的最小值
        for index, row in enumerate(result_matrix):
            result_matrix[index] -= row.min()

        # Step 2: Subtract column mins from each column.从每一列中将去这列的最小值
...
class CoverZeros:
    """
    Use minimum number of lines to cover all zeros in the matrix.使用最少的线覆盖矩阵中的0元素
    Algorithm based on: http://weber.ucsd.edu/~vcrawfor/hungar.pdf 算法的地址
    """
    ...
    def __choice_in_all_marked_columns(self):
        ...
        如果所有标记的列中都有选择，则返回Boolean True。 否则返回布尔值False。"""
